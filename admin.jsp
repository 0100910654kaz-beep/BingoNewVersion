...
<head>
    ...
    <script>
        ...
        // 📺【大進化】スクショに基づいた「3列並び特大プロジェクター大画面」
        ...
        // 5秒に1回の非同期リロード（管理画面全体を書き換え、大画面へ連動）
            ...
            <div class="panel">
                ...
                <ul id="bingoList">
                    <% int rank = 1;
                       for (PlayerResult p : game.getBingoPlayers()) { %>
                        <li><strong><%= rank %>位</strong>: <%= p.getPlayerName() %> さん <span style="color:#e63946; font-weight:bold;">(🔑<%= p.getDrawnNumberAtBingo() %>番 / ⏱️<%= p.getFormattedTime() %>)</span></li>
                    <% rank++; } 
                ...
                <ul id="adminReachList">
                    <% for (PlayerResult p : game.getReachPlayers()) { %>
                        <li><strong><%= p.getPlayerName() %> さん</strong> <span style="color: #ff9800; font-size: 14px; font-weight: bold;">（あと <%= game.getWaitNumbers(p.getPlayerName()) %> 番）</span></li>
                    <% } 
